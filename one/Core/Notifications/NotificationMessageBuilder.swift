...
enum NotificationMessageBuilder {
    ...
    private static func monthlyPortraitVariants(_ ctx: MessageContext) -> [(String, String)] {
        ...
        return [
            ("İşte senin \(monthName) ayın 🎞️", "30 gün, 30 mood — aylık portrenin hazır."),
            ("\(monthName) yansıması ✨", "Bu ayın renk paletini paylaşmaya ne dersin?"),
            ("Aylık posterin hazır 🖼️", "\(monthName) ayını tek bir kartta gör.")
        ]
    }
    ...
    private static func dailyReminderVariants(_ ctx: MessageContext) -> [(String, String)] {
        ...
        if ctx.isHeavyMoodStreak {
            return [
                ("Birkaç gündür yoğunsun 🌫️", "Küçük bir nefes ve tek bir kelime — kendine zaman tanı."),
        ...
        if ctx.isWeekend {
            ...
            case .morning:
                return [
                    ("\(dayLabel) sabahı ☕️", "Yavaş başla — bugünkü vibe ne?"),
                    ("Hafta sonu vibe'ı 🌿", mood.map { "Dün \($0) — bugün hangi tona geçiyorsun?" } ?? "Bugün hangi renktesin?")
                ]
            case .noon, .evening:
                return [
                    ("\(dayLabel) keyfi 🌤️", "Bugünkü mood'unu bırak, kendine zaman ayır."),
        ...
        case 7:
            if hasFriends {
                return [
                    ...
                    ("Bir hafta sessizdin 🕊️", "Çevrende neler oldu görmek ister misin?")
                ]
            }
            return [
                ("Bir haftadır sessiz 🕊️", "Tek bir kelime: bugün nasılsın?"),
