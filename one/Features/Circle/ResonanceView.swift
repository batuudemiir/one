...
// MARK: - Resonance Section View

struct ResonanceSectionView: View {
    @StateObject private var vm: ResonanceViewModel
    @State private var showSendSheet = false
    
    init(shareRecordName: String, receiverID: String) {
        _vm = StateObject(wrappedValue: ResonanceViewModel(
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack {
                Image(systemName: "waveform.circle.fill")
                    .font(.system(size: 17))
                    .foregroundColor(ONETokens.oneBlue)
                Text(NSLocalizedString("resonance.title", comment: "Rezonans"))
                    .bodyLG()
                    .fontWeight(.semibold)
                    .foregroundColor(ONETokens.oneInk)
                
                if !vm.resonances.isEmpty {
                    Text("\(vm.resonances.count)")
                        .monoSM()
                        .fontWeight(.medium)
            ...
            // Received resonances
            if vm.isLoading && vm.resonances.isEmpty {
                HStack {
                    Spacer()
                    ProgressView()
                        .scaleEffect(0.8)
                ...
                .padding(.vertical, 8)
            } else if !vm.resonances.isEmpty {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 10) {
                        ForEach(vm.resonances) { r in
                            ResonanceBubble(resonance: r)
                        }
                    }
                }
            }
            ...
            if vm.hasSent {
                HStack(spacing: 8) {
                    Image(systemName: "checkmark.circle.fill")
                        ...
                        .bodySM()
                    Text(NSLocalizedString("resonance.sent", comment: "Rezonansın gönderildi"))
                        .bodySM()
                        .fontWeight(.medium)
                        .foregroundColor(ONETokens.oneGreen)
                }
                ...
                Button(action: { showSendSheet = true }) {
                    HStack(spacing: 10) {
                        Image(systemName: "waveform.path")
                            .bodySM()
                            .fontWeight(.medium)
                        Text(NSLocalizedString("resonance.send", comment: "Rezonans Gönder"))
                            .bodySM()
                            .fontWeight(.semibold)
                    }
        ...
        .animation(.easeInOut(duration: 0.25), value: vm.hasSent)
        .animation(.easeInOut(duration: 0.25), value: vm.resonances.count)
        .onAppear { vm.load() }
        .sheet(isPresented: $showSendSheet) {
            ResonanceSendSheet(vm: vm)
