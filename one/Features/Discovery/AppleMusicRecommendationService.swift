...
class AppleMusicRecommendationService {
    ...
    func getRecommendations(profile: TasteProfile, limit: Int = 8) async throws -> [SongRecommendation] {
        ...
        // Randomize genre and artist selection for variety
        let shuffledGenres = profile.topGenres.shuffled()
        ...
        if !shuffledGenres.isEmpty {
            let genreRecommendations = try await searchByGenres(
                genres: Array(shuffledGenres.prefix(2)),
                limit: limit
            )
        ...
        if !shuffledArtists.isEmpty && recommendations.count < limit * 2 {
            let artistRecommendations = try await searchByArtists(
                artists: Array(shuffledArtists.prefix(2)),
                limit: limit
            )
    ...
    private func searchByArtists(artists: [String], limit: Int) async throws -> [SongRecommendation] {
        ...
        for artistName in artists {
            do {
                ...
                searchRequest.limit = limit / artists.count
