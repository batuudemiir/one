...
nonisolated enum QuoteModeOptions {
    ...
    static func make(paths: [QuotePath], userPaths: [String], lang: String,
        ...
        // Adım adım: tek bir `+` zinciri derleyiciyi zorluyor.
        var result: [QuoteModeOption] = [
            option(.forYou, NSLocalizedString("one2.quotes.mode.forYou", comment: "Quotes feed mode: personalised"), .main)
        ]
        result += (own + others).map { option(.path($0.id), $0.title, .path) }
        result.append(option(.favorites, NSLocalizedString("one2.quotes.mode.favorites", comment: "Quotes feed mode: liked quotes"), .list))
        result.append(option(.written, NSLocalizedString("one2.quotes.mode.written", comment: "Quotes feed mode: quotes the user wrote about"), .list))
        return result
    }
